...
| Linux client | `0.1.0-alpha.2` | Exact Core `0.1.0-alpha.2`; ABI `1`; wire protocol 1; multi-profile/model persistence; derived onboarding; session credentials; GIO Secret Service adapter with a valid single-layer plain-string OpenSession payload, isolated real-daemon CRUD/cleanup, persistent daemon-restart/locked-item fail-closed lifecycle, and secure persistent-credential onboarding verification, plus session-only fallback; no-credential OpenAI-compatible loopback provider connection and streaming; runtime English/Simplified Chinese PO action labels; generic completion desktop notification without source/translated content; bounded native UTF-8 TXT/Markdown import up to 4 MiB through FileDialog or single-file source-editor drag-and-drop; post-startup storage degradation; baseline GTK accessibility semantics on GTK 4.10+; pinned Flatpak packaging scaffold with GNOME 48 SDK build and prerelease CI bundle | Functional source `7d7eba9960b657f0460fb0daaaaebaaa609f39b1` passed native CI; packaging revision `fd1f400` passed `Flatpak Linux` run `29606612834` and uploaded artifact `8417198959`; sandbox, portal/notification delivery, signing, and distributable release remain open, unreleased |
...
without source or translated content; notification-server delivery and packaging remain runtime
