...
| Localization | `0.1.0` | Message schema `1.0.0`; resource contract `1`; 286 canonical messages including Linux-only status, partial-output, text-import, glossary CSV import/export, PDF fidelity and subtitle readability warnings, document-job actions/dialog/status/tooltip controls, exported-output open/failure actions, Incognito privacy controls, history and translation-memory controls/status, translation-export, provider-profile, source-target, onboarding-stage, active-provider, notification, draft-note, locale selector language names, fixed provider/file/worker errors, reducer-state/category guidance, fixed worker/file/storage/provider-error guidance, construction-stage provider/default-control and request-level glossary copy, and Core/loopback startup plus profile-storage error copy; paired PO/MO Linux resources | CI-verified development bundle at l10n evidence revision `273be8a4e9c3b1084f393ce0086cdf2c42fcd4e9`, 59 generated artifacts, deterministic bundle SHA-256 `ee7c269571beca22cdbd7bea971ae266975b8004490b02ead4b71305e3a93872`, unreleased |
...
Linux-first keyboard traversal checkpoint: Linux `abdbe78` adds a runtime-only GTK focus probe and
an Xvfb/xfwm4 fixture that injects Tab and Shift+Tab events across the tested onboarding/workspace
controls. Provider fields are asserted enabled, mapped, and focusable, while their omission from
the default Tab chain remains an explicit Linux follow-up. Native `29661215896` (job `88124076970`),
Foundation `29661215890`, and Flatpak `29661215925` (job `88124076966`) passed; provider-form Tab-chain repair,
screen-reader narration, physical desktop review, OCR, other platform clients, and stable-release
