...
| Linux client | `0.1.0-alpha.2` | GTK client with provider/routing/document slices at Linux `ab82f36`, Core `115535c`, and l10n `737d890e`; explainable routing diagnostics expose eligible/rejected candidates, stable rejection reasons, ranking inputs, and fallback order without secrets; Automatic quality-ranked retryable fallback and the explicit GTK fallback-approval lifecycle are covered by worker/UI regressions; final code-head push/PR Native, Flatpak, and Foundation gates passed | Unreleased |
