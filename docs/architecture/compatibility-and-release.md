...
`c559b32d3869e01983f2bbf32f1386bad99c3290` and Linux functional revision
`b38a8fd722d4740abd161e30197354793e3de1f6`, both at `0.1.0-alpha.2`, ABI 1, and wire protocol 1.
Native clients query `CompatibilitySnapshot` through `lm_engine_get_compatibility` before provider
work; file-lease capabilities remain unprojected.
...
import uses a bounded UTF-8 GTK FileDialog/GIO path for TXT/Markdown/CSV/JSON/HTML/SRT/WebVTT plus bounded DOCX/PPTX/XLSX/EPUB packages and text-based PDF pages, preserving PDF page association and available coordinates with a structured HTML fallback for unsafe target encodings, structured page-level warnings for image-only pages, uncertain reading order, and limited reconstruction, cue-level subtitle line-length and reading-speed warnings with configurable limits, and a URI-list/GIO source-editor DropTarget; the GTK queue lists persisted document snapshots with progress/state metadata and rebinds a selected job to the existing controls;
