...
| 4–5 — Providers and quality | Linux catalog, local models, routing, fallback, glossary, chunking, history, memory, presets, and bounded proxy transport settings have deterministic evidence | Cross-client parity, live-provider interoperability, proxy authentication, provider-specific semantics |
...
1. Keep the Linux PR and central issue synchronized with the latest verified heads, including the proxy-settings checkpoint.
