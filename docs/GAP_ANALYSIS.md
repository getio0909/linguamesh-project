...
| 4–5 — Providers and quality | Linux catalog, local models, routing, fallback, glossary, chunking, history, memory, presets, bounded proxy transport settings, and bounded total/connection timeout settings have deterministic evidence | Cross-client parity, live-provider interoperability, proxy authentication, streaming-idle/TLS timeout policy, provider-specific semantics |
