...
| 4–5 — Providers and quality | Linux catalog, local models, routing, fallback, glossary, chunking, history, memory, presets, bounded proxy transport settings, bounded total/connection/streaming-idle timeout settings have deterministic evidence | Cross-client parity, live-provider interoperability, proxy authentication, TLS policy, provider-specific semantics |
