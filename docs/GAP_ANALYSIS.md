...
| 4–5 — Providers and quality | Linux catalog, local models, routing, fallback, glossary, chunking, history, memory, presets, bounded proxy transport settings, bounded total/connection/streaming-idle timeout settings, and additive custom PEM trust bundles have deterministic evidence | Cross-client parity, live-provider interoperability, proxy authentication, client-certificate TLS policy, provider-specific semantics |
...
1. Keep the Linux PR and central issue synchronized with the latest verified heads, including the proxy-settings, timeout, and custom-trust-bundle checkpoints.
