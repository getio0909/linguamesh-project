...
| Localization bundle | Validated locally and remotely | l10n evidence revision `0ed08e9e5040edb8f981c90c82dff0fd30219e25` (content revision `0536ae1695116f5130943c1114bc31c60376407e`) contains 107 canonical messages, including Linux-only status, partial-output, text-import, provider-profile, source-target, onboarding-stage, active-provider, notification, and draft-note guidance, 12 official locale packs, two pseudo-locales, 45 generated artifacts, 25 passing tests, platform-format checks, and deterministic bundle ZIP SHA-256 `9eb7acda3347cf9fede9eadf158cda2c233cfbd8399b45ffc4497f0802e32777`. Non-English packs remain explicitly unreviewed drafts. |
| Native Linux alpha.2 slice | Validated locally and remotely | Functional revision `7d7eba9960b657f0460fb0daaaaebaaa609f39b1` starts disconnected, derives provider setup from authoritative state, manages multiple profiles with independent models, preserves authenticated A/B routing without credential crossover, exposes baseline GTK accessibility semantics, uses a GIO Secret Service adapter with a corrected single-layer plain-string OpenSession Variant plus isolated real-daemon CRUD/cleanup, persistent daemon-restart restoration, locked-item fail-closed verification, secure persistent-credential onboarding, a no-credential OpenAI-compatible loopback path, and session fallback, exposes twelve official Linux PO packs with runtime action-label, workspace-widget, active-provider, active-provider transition/mode, status summary/partial-output, text-file import, provider-profile, source/target language, onboarding-stage, draft-note, and theme-option switching plus Arabic RTL root direction, preserves the source editor buffer across a Simplified Chinese-to-Arabic locale switch, emits generic completion notifications without source or translated content, and imports bounded UTF-8 TXT/Markdown text through GTK FileDialog/GIO or single-file source-editor drag-and-drop. Post-startup persistent Connect, model-update, and deletion `ENOSPC` failures reject before success, preserve the prior session, and restore only pre-fault state. Local suites passed 45 portable tests, 4 localization tests, and one exact fault test. Evidence head `6c8b552` (l10n evidence pin) passed Native Linux run `29627288199`, Foundation run `29627288206`, and Flatpak run `29627288201`; the prior functional evidence head `220add98a1fcebe0392234db08ce1b9f266d095e` passed Native Linux run `29627011700`, Foundation run `29627011696`, and Flatpak run `29627011708`; the native gate includes GTK locale/widget/theme/status/text-import/provider-profile/source-target/onboarding-stage/active-provider/notification assertions, source-buffer preservation, real GTK locale/RTL code, Secret Service prompted-flow store/delete rejection, real portal backend, application FileDialog, source-editor URI-list drag/drop, dunst delivery, and visible viewable Dunst desktop-shell window fixtures. End-user prompt approval, physical compositor/GPU rendering, and complete visible-string gettext coverage remain unverified. |
...
| GitHub Actions | Passed | Linux functional run `29604269568` (job `87963611054`) and foundation run `29604269516` passed the exact fault test, persistent Secret Service lifecycle and secure-onboarding fixtures, no-credential loopback regression, GTK assertions, both display gates, bounded import path, and the all-feature build. Linux evidence head `220add98a1fcebe0392234db08ce1b9f266d095e` passed Native Linux run `29627011700`, Foundation run `29627011696`, and Flatpak run `29627011708`; the native job also verified the pinned 107-message localization snapshot, Arabic RTL direction, source-buffer preservation, localized workspace widgets, active-provider/status-summary/partial-output/text-import/provider-profile/source-target/onboarding-stage/active-provider/notification/theme-option localization, prompted-flow store/delete rejection, real portal backend, application FileDialog callback, source-editor drag/drop gesture, dunst delivery, and visible viewable Dunst window. Central integration commit `35ee2d5` passed coordination run `29627110594` with Linux job `88033588032` and PowerShell job `88033588065`. The earlier failed status-localization attempts were corrected by updating the workflow localization pin. |
