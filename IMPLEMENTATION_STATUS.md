...
adapter, and runtime PO action-label slice are
...
| Native Linux alpha.2 slice | Validated locally and remotely | Functional revision `1dfe2bcac684696ee55f56e625fcf89ffcb1a6dd` starts disconnected, derives provider setup from authoritative state, manages multiple profiles with independent models, preserves authenticated A/B routing without credential crossover, exposes baseline GTK accessibility semantics, uses a GIO Secret Service adapter for persistent SecretRef values with fail-closed session fallback, and loads pinned English/Simplified Chinese PO catalogs for runtime Translate/Stop labels. Post-startup persistent Connect, model-update, and deletion `ENOSPC` failures reject before success, preserve the prior session, and restore only pre-fault state. Local suites passed 41 and 67 tests plus one exact fault test. Validation head `1dfe2bcac684696ee55f56e625fcf89ffcb1a6dd` passed Native Linux run `29593874961` (job `87929412911`) with both display gates, strict Clippy, storage-fault test, localization assertions, and the native build. Real desktop keyring CRUD and complete UI gettext coverage remain unverified. |
| GitHub Actions | Passed | Runtime localization validation run `29593874961` (job `87929412911`) and evidence-head run `29594014033` (job `87929863334`) passed the exact fault test, GTK assertions, both display gates, catalog label switching, and the all-feature build; Foundation runs `29593874763` and `29594014184` also passed. Central integration commit `a964ac060c51665793ed455763137ee30bd22a26` passed coordination run `29594286528`, Linux job `87930776189`, and PowerShell job `87930776107`. |
| Non-functional repository heads | Passed | Core head `b8ce1fda3a2a20ea2bc99bb6abff70e1a7a95383` is a documentation-only descendant of the functional pin; Core run `29573340721` and Native SDK run `29573340735` passed. Linux evidence head `bc827d55118199e2e64e063a106da281f8a8bdf1` changes only evidence documentation relative to functional source `1dfe2bcac684696ee55f56e625fcf89ffcb1a6dd`; foundation run `29594014184` and Native Linux run `29594014033` (job `87929863334`) passed. The release manifest intentionally pins functional revisions. |
...
coverage, secure persistent-credential onboarding, database faults beyond the verified `ENOSPC` transaction boundary, packaging,
