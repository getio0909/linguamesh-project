...
- Re-read the task attachment and completely re-read `PROJECT_GOAL.md`, `AGENTS.md`, and `PLANS.md`.
  At audit time, repository heads were central `e7511c6ae553d6144360309e20834e71dd3b4938`, Linux
...
- Linux status head `8ab718a2174d65a6ec33a1b4bcc3d8a4c52db48b` passed localization audits, Flatpak
  metadata, formatting, locked offline checks, strict Clippy, no-default tests (`85 passed;
  1 ignored`), and demo-provider tests (`168 passed; 7 ignored`). Core's locked offline workspace
  tests passed 244 unit tests plus doc-test targets.
- Local client-certificate, OCR, storage-fault, and Secret Service prompt fixtures passed. The
  document portal remains unavailable locally because FUSE cannot mount the user document path;
  Xvfb/Weston are absent, so display-backed checks remain hosted-CI evidence.
- Native push/Flatpak/Foundation `30133558944`/`30133558901`/`30133558885` and PR
  Native/Flatpak/Foundation `30133561000`/`30133561001`/`30133561031` all passed.
- Native push run `30133558944` produced unexpired artifact
  `linguamesh-linux-native-evidence-8ab718a2174d65a6ec33a1b4bcc3d8a4c52db48b` (artifact ID
  `8612069642`). `SHA256SUMS` verified binary
  `35c1878648dc81db3f474c24a0dbfa8fcd6bd0840fafef453ff686eeaadc52e9` and source archive
  `d8374e1c79d390c4b7b76795cc34e29fc6e5417ac78f6b50c09407b37948f605`; the bundle has a
  234-package SPDX-2.3 SBOM and a 140-entry source archive. It remains unsigned prerelease
  evidence with machine-specific performance data.
- Release remains `unreleased`; physical power-loss, arbitrary third-party VFS, human/physical
  review, other clients, signing, rollback authorization, distributable promotion, and stable
  release evidence remain open.

## 2026-07-24 — Linux final current-head Native evidence artifact verification

Assumption: a downloadable CI evidence bundle proves reproducible prerelease packaging inputs and
checksums, but unsigned artifacts and machine-specific performance data do not authorize a stable
release.

- Linux documentation/status head `fd89f6e4e4c1d52b1634426005c7ff0ccf611e56` is published.
- Native push run `30132806085` produced the unexpired artifact
  `linguamesh-linux-native-evidence-fd89f6e4e4c1d52b1634426005c7ff0ccf611e56` (artifact ID
  `8611818432`). Download-time `SHA256SUMS` verification passed for the native binary
  `35c1878648dc81db3f474c24a0dbfa8fcd6bd0840fafef453ff686eeaadc52e9` and source archive
  `c00609605de3a4012194c01453e697b79b20d0ac98da4a0a1595876879faaaec`.
- The bundle contains a 234-package SPDX-2.3 SBOM and a 140-entry source archive. Its build and
  rollback records classify it as unsigned release-mode prerelease evidence with no stable
  revision, signing key, or release authorization; its performance file is machine-specific.
- Push Native/Flatpak/Foundation `30132806085`/`30132806113`/`30132806111` and PR
...
- The bundle contains a 234-package SPDX-2.3 SBOM and a 140-entry source archive. Its build and
  rollback records classify it as unsigned release-mode prerelease evidence with no stable
  revision, signing key, or release authorization.
- The performance file is a machine-specific CI baseline, not a cross-machine claim. Release
  remains `unreleased`; signing, rollback authorization, distributable promotion, and stable-release
...
they do not replace live enterprise interoperability, human prompt review, or display-backed keyring
UI evidence.

- Linux status head `084ba101b07206aebd890258b3b61967eaf5fe15` passed
  `bash tools/run-client-certificate-interop-test.sh` with all four HTTPS tests exactly once:
  trusted client authentication plus untrusted-server, hostname-mismatch, and untrusted-client-CA
  rejection.
...
Assumption: the dedicated OCR and private-mount storage-fault runners are reproducible Linux
...
- Linux source/test head `41a452b4453697d41d1799ad654d331fb05482ff` adds
  `published_storage_does_not_follow_replaced_database_sidecars`, covering both sidecar pathnames
  and requiring either a fail-closed write or unchanged replacement bytes.
- Local formatting, all-target checking, strict Clippy, and the demo-provider suite passed
  (`168 passed; 7 ignored`). The focused GUI binary remains linker-limited by missing GTK/GDK/
  Graphene symbols on this host.
- Packaging/status head `759a828f4db4efcd17dfc6b4e25aa3a35e57b1ac` repins Flatpak after the first
  push/PR metadata guards `30127637300`/`30127639383` rejected the stale source pin before SDK
  build. Replacement push Native/Flatpak/Foundation `30127781035`/`30127780961`/`30127781067`
  and PR `30127783125`/`30127783122`/`30127783152` all passed; release remains `unreleased`.
- Broader filesystem/VFS, physical power-loss, cross-client, live-provider, human/physical,
...
The documented clean-bootstrap command was rerun from a temporary workspace using a shallow clone
of central `main` and `GITHUB_OWNER=getio0909`. All seven public canonical repositories cloned;
...
- Linux runtime test `f6cdb44dd6e411c2fab1c9f39cd3cd63361a1352` extends the GTK
...
- Linux packaging/docs commit `d1e7368edcf8426d3986165fc5b2adbd33cabe48` pins Flatpak to the
  tested runtime and documents the expanded fixture. Linux status head is
  `e9b7d80e3ecac045eeb10b37ea59871c0ada6198`; local formatting, locked all-target/test checks,
  strict Clippy, 163 demo-provider tests with three documented ignores, localization audits,
  synchronization, Flatpak metadata, and diff checks passed. The host GTK linker limitation
  remains confined to the focused display-backed binary.
- Final status-head push Native/Flatpak/Foundation runs `30039406785`/`30039406821`/
  `30039406753` and PR Native/Flatpak/Foundation runs `30039409345`/`30039409381`/
...
Assumption: the process-kill fixture is bounded crash-durability evidence only; it does not
stand in for physical power-loss, filesystem-specific recovery, or alternate GIO/VFS behavior.

- Linux runtime `361ac7ba9d6a18c26de4487ab424d6500fbbeafd` adds a test-only barrier immediately
