...
| Native Linux alpha.2 slice | Validated locally and remotely | Functional revision `d6bd2bd06ccdf04f3aead0c7f1da5ba74f84c550` starts disconnected, derives provider setup from authoritative state, manages multiple credential-free profiles with independent models, preserves authenticated A/B routing without credential crossover, and exposes baseline GTK accessibility semantics on GTK 4.10+. Post-startup persistent Connect, model-update, and deletion `ENOSPC` failures reject before success, preserve the prior session, and restore only pre-fault state. Local suites passed 41 and 66 tests plus one exact fault test. Native Linux run `29589043315` (job `87913221576`) passed the controlled Ubuntu mount fallback, accessibility assertions, both GTK display gates, strict Clippy, and the native build. Persistent secrets still fail closed. |
| GitHub Actions | Passed | Core run `29572377637` (Rust job `87858924329`) and Native SDK run `29572377631` (Linux job `87858924315`) passed for the functional Core source. Linux accessibility run `29589043315` (job `87913221576`) and evidence-head run `29589332282` (job `87914189125`) passed the exact fault test, accessibility assertions, both display gates, and the all-feature build. Central integration revision `04c0e07ccb769a9e67e80ee76042d6c568a7f8d4` passed coordination run `29587437567`, Linux job `87907811474`, and PowerShell job `87907811494`; prior repository and localization evidence remains recorded in component status files. |
| Non-functional repository heads | Passed | Core head `b8ce1fda3a2a20ea2bc99bb6abff70e1a7a95383` is a documentation-only descendant of the functional pin; Core run `29573340721` and Native SDK run `29573340735` passed. Linux evidence head `07d0b4daddf04d369768893a531276d507292356` changes only evidence documentation relative to functional source `d6bd2bd06ccdf04f3aead0c7f1da5ba74f84c550`; foundation run `29589332132` and Native Linux run `29589332282` (job `87914189125`) passed. The release manifest intentionally pins functional revisions. |
