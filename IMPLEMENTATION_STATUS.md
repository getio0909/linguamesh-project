...
Flatpak manifest now has a remotely verified GNOME 48 SDK build and prerelease CI bundle; sandbox
...
| Linux Flatpak packaging scaffold | GNOME 48 SDK build and CI bundle validated | Linux packaging revision `fd1f400058f4c68b47a9bd0823e790c6d9cef263` publishes the pinned GNOME 48 manifest, immutable Core/Linux source pins, generated Cargo archive hashes, desktop entry, AppStream metadata, icon, and constrained runtime permissions. `bash tools/validate-flatpak-metadata.sh` passed locally; `Flatpak Linux` run `29606612834` (job `87971271146`) built `linguamesh-linux-x86_64-x86_64.flatpak` and uploaded artifact `8417198959` (2,390,377 bytes). Sandbox launch, portal/notification delivery, signing, and distributable release remain unverified. |
| GitHub Actions | Passed | Linux functional run `29604269568` (job `87963611054`) and foundation run `29604269516` passed the exact fault test, persistent Secret Service lifecycle and secure-onboarding fixtures, no-credential loopback regression, GTK assertions, both display gates, bounded import path, and the all-feature build. Linux documentation evidence head `a255b039ce37bcb2b362cfeb3d34a6283ed2aad5` passed Native Linux run `29604459967` and foundation run `29604459891`. Packaging head `302ba8c` passed Native Linux run `29605298735` and foundation run `29605298772`; the current Flatpak packaging revision `fd1f400` passed the dedicated `Flatpak Linux` run `29606612834` with job `87971271146`. Central integration commit `a7ca1f84eef3f98647a4383dc931c11049171b63` passed coordination run `29604649153` with Linux job `87964871597` and PowerShell job `87964871588`; the packaging documentation checkpoint passed coordination run `29605424763` with Linux job `87967368496` and PowerShell job `87967368478`. |
| Non-functional repository heads | Passed | Core head `b8ce1fda3a2a20ea2bc99bb6abff70e1a7a95383` is a documentation-only descendant of the functional pin; Core run `29573340721` and Native SDK run `29573340735` passed. Linux evidence head `a255b039ce37bcb2b362cfeb3d34a6283ed2aad5` changes only evidence documentation relative to functional source `7d7eba9960b657f0460fb0daaaaebaaa609f39b1`; foundation run `29604459891` and Native Linux run `29604459967` (job `87964249312`) passed. Packaging revision `fd1f400` passed the remote GNOME 48 SDK build and uploaded a prerelease CI bundle; the release manifest intentionally pins functional revisions and lists no release artifact. |
