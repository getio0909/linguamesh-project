...
| Native Linux alpha.2 slice | Validated locally and remotely | Functional revision `96d34a5448d0f718fd87c68e88129c05fed43ee5` starts disconnected, derives provider setup from authoritative state, manages multiple profiles with independent models, preserves authenticated A/B routing without credential crossover, exposes baseline GTK accessibility semantics, uses a GIO Secret Service adapter for persistent SecretRef values with fail-closed session fallback, loads pinned English/Simplified Chinese PO catalogs for runtime Translate/Stop labels, emits generic completion notifications without source or translated content, and imports bounded UTF-8 TXT/Markdown text through GTK FileDialog/GIO. Post-startup persistent Connect, model-update, and deletion `ENOSPC` failures reject before success, preserve the prior session, and restore only pre-fault state. Local suites passed 44 and 70 tests plus one exact fault test. Validation revision `96d34a5448d0f718fd87c68e88129c05fed43ee5` passed Native Linux run `29596052224` (job `87936587342`) with both display gates, strict Clippy, storage-fault test, text-import control assertions, and the native build. Real desktop keyring CRUD, notification-server delivery, interactive portal leases, drag-and-drop, and complete UI gettext coverage remain unverified. |
| GitHub Actions | Passed | Text-import validation run `29596052224` (job `87936587342`) and evidence-head run `29596239167` (job `87937214530`) passed the exact fault test, GTK assertions, both display gates, bounded import path, and the all-feature build; Foundation runs `29596052213` and `29596239653` also passed. Central integration commit `b4d8b3c40b49f0ba327beb979d1b4b9fdbf6fdda` passed coordination run `29596541373`, Linux job `87938213706`, and PowerShell job `87938213592`. |
| Non-functional repository heads | Passed | Core head `b8ce1fda3a2a20ea2bc99bb6abff70e1a7a95383` is a documentation-only descendant of the functional pin; Core run `29573340721` and Native SDK run `29573340735` passed. Linux evidence head `1553841dc527f1f3bc9556ce5f88650f28221d85` changes only evidence documentation relative to functional source `96d34a5448d0f718fd87c68e88129c05fed43ee5`; foundation run `29596239653` and Native Linux run `29596239167` (job `87937214530`) passed. The release manifest intentionally pins functional revisions. |
