...
sandbox startup smoke; physical desktop-shell notification rendering and release artifacts remain
open. No stable product
...
| Native Linux alpha.2 slice | Validated locally and remotely | Functional revision `7d7eba9960b657f0460fb0daaaaebaaa609f39b1` starts disconnected, derives provider setup from authoritative state, manages multiple profiles with independent models, preserves authenticated A/B routing without credential crossover, exposes baseline GTK accessibility semantics, uses a GIO Secret Service adapter with a corrected single-layer plain-string OpenSession Variant plus isolated real-daemon CRUD/cleanup, persistent daemon-restart restoration, locked-item fail-closed verification, secure persistent-credential onboarding, a no-credential OpenAI-compatible loopback path, and session fallback, loads pinned English/Simplified Chinese PO catalogs for runtime Translate/Stop labels, emits generic completion notifications without source or translated content, and imports bounded UTF-8 TXT/Markdown text through GTK FileDialog/GIO or single-file source-editor drag-and-drop. Post-startup persistent Connect, model-update, and deletion `ENOSPC` failures reject before success, preserve the prior session, and restore only pre-fault state. Local suites passed 44 and 71 tests plus one exact fault test. Evidence head `d2fb3abe6c1505385850cd2a1053a7f64dbd7d0e` passed Native Linux run `29619390971`, Foundation run `29619390964`, and Flatpak run `29619390913`; the native gate includes real XDG document-portal, interactive portal backend, application FileDialog callback, source-editor URI-list drag/drop, and dunst delivery fixtures. Prompted interactive flows, physical desktop-shell notification rendering, and complete UI gettext coverage remain unverified. |
| Linux Flatpak packaging scaffold | GNOME 49 SDK build and bounded sandbox startup validated | Linux packaging revision `fd1f400058f4c68b47a9bd0823e790c6d9cef263` publishes the pinned GNOME 49 manifest, immutable Core/Linux source pins, generated Cargo archive hashes, desktop entry, AppStream metadata, icon, and constrained runtime permissions. `bash tools/validate-flatpak-metadata.sh` passed locally; latest `Flatpak Linux` run `29619390913` is the current remote packaging gate. Physical desktop-shell notification rendering, signing, and distributable release remain unverified. |
| GitHub Actions | Passed | Linux functional run `29604269568` (job `87963611054`) and foundation run `29604269516` passed the exact fault test, persistent Secret Service lifecycle and secure-onboarding fixtures, no-credential loopback regression, GTK assertions, both display gates, bounded import path, and the all-feature build. Linux evidence head `d2fb3abe6c1505385850cd2a1053a7f64dbd7d0e` passed Native Linux run `29619390971`, Foundation run `29619390964`, and Flatpak run `29619390913`; the native job verified the real portal backend, application FileDialog callback, source-editor drag/drop gesture, dunst delivery, and document-portal lease fixture. Central integration commit `0de200a` passed coordination run `29619564326` with Linux job `88011719149` and PowerShell job `88011719080`. |
| Non-functional repository heads | Passed | Core head `b8ce1fda3a2a20ea2bc99bb6abff70e1a7a95383` is a documentation-only descendant of the functional pin; Core run `29573340721` and Native SDK run `29573340735` passed. Linux evidence head `d2fb3abe6c1505385850cd2a1053a7f64dbd7d0e` adds application FileDialog and source-editor drag/drop fixtures plus evidence documentation relative to functional source `7d7eba9960b657f0460fb0daaaaebaaa609f39b1`; latest Native, Foundation, and Flatpak gates are recorded above. The release manifest intentionally pins functional revisions and lists no release artifact. |
...
coverage, physical desktop-shell notification rendering,
