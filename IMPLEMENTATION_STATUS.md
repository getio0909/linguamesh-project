...
Flatpak manifest now has a remotely verified GNOME 49 SDK build and prerelease CI bundle; sandbox
smoke, portal/notification delivery, and release artifacts remain open. No stable product
...
| Native Linux alpha.2 slice | Validated locally and remotely | Functional revision `7d7eba9960b657f0460fb0daaaaebaaa609f39b1` starts disconnected, derives provider setup from authoritative state, manages multiple profiles with independent models, preserves authenticated A/B routing without credential crossover, exposes baseline GTK accessibility semantics, uses a GIO Secret Service adapter with a corrected single-layer plain-string OpenSession Variant plus isolated real-daemon CRUD/cleanup, persistent daemon-restart restoration, locked-item fail-closed verification, secure persistent-credential onboarding, a no-credential OpenAI-compatible loopback path, and session fallback, loads pinned English/Simplified Chinese PO catalogs for runtime Translate/Stop labels, emits generic completion notifications without source or translated content, and imports bounded UTF-8 TXT/Markdown text through GTK FileDialog/GIO or single-file source-editor drag-and-drop. Post-startup persistent Connect, model-update, and deletion `ENOSPC` failures reject before success, preserve the prior session, and restore only pre-fault state. Local suites passed 44 and 71 tests plus one exact fault test. Native Linux run `29604269568` (job `87963611054`) passed the worker and GTK secure-onboarding fixture, no-credential loopback regression, real GTK tests under X11/Xvfb and forced Wayland/headless Weston, storage-fault gate, Secret Service fixture, and native build. Prompted interactive flows, notification-server delivery, interactive portal leases, and complete UI gettext coverage remain unverified. |
| Linux Flatpak packaging scaffold | GNOME 49 SDK build and CI bundle validated | Linux packaging revision `fd1f400058f4c68b47a9bd0823e790c6d9cef263` publishes the pinned GNOME 49 manifest, immutable Core/Linux source pins, generated Cargo archive hashes, desktop entry, AppStream metadata, icon, and constrained runtime permissions. `bash tools/validate-flatpak-metadata.sh` passed locally; `Flatpak Linux` run `29607653864` (job `87974679904`) built `linguamesh-linux-x86_64-x86_64.flatpak` and uploaded artifact `8417588673` (2,396,371 bytes). Sandbox launch, portal/notification delivery, signing, and distributable release remain unverified. |
| GitHub Actions | Passed | Linux functional run `29604269568` (job `87963611054`) and foundation run `29604269516` passed the exact fault test, persistent Secret Service lifecycle and secure-onboarding fixtures, no-credential loopback regression, GTK assertions, both display gates, bounded import path, and the all-feature build. Linux documentation evidence head `a255b039ce37bcb2b362cfeb3d34a6283ed2aad5` passed Native Linux run `29604459967` and foundation run `29604459891`. Packaging head `302ba8c` passed Native Linux run `29605298735` and foundation run `29605298772`; the GNOME 49 Flatpak packaging revision `fd1f400` passed `Flatpak Linux` run `29607653864` with job `87974679904`, while the same commit passed Native Linux run `29607653918` and Foundation run `29607653880`. Central integration commit `a7ca1f84eef3f98647a4383dc931c11049171b63` passed coordination run `29604649153` with Linux job `87964871597` and PowerShell job `87964871588`; the packaging documentation checkpoint passed coordination run `29605424763` with Linux job `87967368496` and PowerShell job `87967368478`. |
| Non-functional repository heads | Passed | Core head `b8ce1fda3a2a20ea2bc99bb6abff70e1a7a95383` is a documentation-only descendant of the functional pin; Core run `29573340721` and Native SDK run `29573340735` passed. Linux evidence head `a255b039ce37bcb2b362cfeb3d34a6283ed2aad5` changes only evidence documentation relative to functional source `7d7eba9960b657f0460fb0daaaaebaaa609f39b1`; foundation run `29604459891` and Native Linux run `29604459967` (job `87964249312`) passed. Packaging revision `fd1f400` passed the GNOME 49 SDK build and uploaded a prerelease CI bundle; the release manifest intentionally pins functional revisions and lists no release artifact. |
...
database faults beyond the verified `ENOSPC` transaction boundary, Flatpak sandbox smoke,
portal/notification delivery, release signing and distributable artifacts, AT-SPI/Orca,
