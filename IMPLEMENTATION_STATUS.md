...
| Localization bundle | Validated locally and remotely | l10n evidence revision `08118b498646ebf56cbb072b937d95fceb34b75c` contains 148 canonical messages, including Linux-only status, partial-output, text-import, provider-profile, source-target, onboarding-stage, active-provider, notification, draft-note, fixed provider/file/worker errors, and reducer-state/category error guidance, 12 official locale packs, two pseudo-locales, 45 generated artifacts, 25 passing tests, platform-format checks, and deterministic bundle ZIP SHA-256 `08d252c8d3fbdd163e7a5f18eea06ac806029667b8c2ec1e836245d66c2b2595`. Non-English packs remain explicitly unreviewed drafts. |
| Native Linux alpha.2 slice | Validated locally and remotely | Functional revision `9f21836f214d3056934fac9322adc0f20791834e` starts disconnected, derives provider setup from authoritative state, manages multiple profiles with independent models, preserves authenticated A/B routing without credential crossover, exposes baseline GTK accessibility semantics, uses a GIO Secret Service adapter with a corrected single-layer plain-string OpenSession Variant plus isolated real-daemon CRUD/cleanup, persistent daemon-restart restoration, locked-item fail-closed verification, secure persistent-credential onboarding, a no-credential OpenAI-compatible loopback path, and session fallback, exposes twelve official Linux PO packs with runtime action-label, workspace-widget, active-provider, active-provider transition/mode, status summary/partial-output, text-file import, provider-profile, source/target language, onboarding-stage, draft-note, fixed provider/file/worker and reducer-state/category error messages, and theme-option switching plus Arabic RTL root direction, preserves the source editor buffer across a Simplified Chinese-to-Arabic locale switch, emits generic completion notifications without source or translated content, and imports bounded UTF-8 TXT/Markdown text through GTK FileDialog/GIO or single-file source-editor drag-and-drop. Post-startup persistent Connect, model-update, and deletion `ENOSPC` failures reject before success, preserve the prior session, and restore only pre-fault state. Local suites passed 50 portable tests, 4 localization tests, and one localized-error test. Evidence head `9f21836f214d3056934fac9322adc0f20791834e` passed Native Linux run `29628307915`, Foundation run `29628307945`, and Flatpak run `29628307886`; the native gate includes GTK locale/widget/theme/status/text-import/provider-profile/source-target/onboarding-stage/active-provider/notification/error assertions, source-buffer preservation, real GTK locale/RTL code, Secret Service prompted-flow store/delete rejection, real portal backend, application FileDialog, source-editor URI-list drag/drop, dunst delivery, and visible viewable Dunst desktop-shell window fixtures. End-user prompt approval, physical compositor/GPU rendering, and complete visible-string gettext coverage remain unverified. |
...
| GitHub Actions | Passed | Linux functional run `29628307915` and foundation run `29628307945` passed the exact fault test, persistent Secret Service lifecycle and secure-onboarding fixtures, no-credential loopback regression, GTK assertions, both display gates, bounded import path, reducer-state localization, and the all-feature build; Flatpak run `29628307886` passed the GNOME 49 SDK build and smoke gate. Central integration commit `522c94e` passed coordination run `29627851127`; the current coordination run will be recorded after these manifest and evidence updates. |
