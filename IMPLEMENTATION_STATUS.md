...
| Localization bundle | Validated locally and remotely | l10n evidence revision `738a7c7328f24acc12c15be78bb11737220bbbae` contains 267 canonical messages, including Linux translation-memory controls/status, PDF fidelity warnings, and subtitle readability warnings, 12 official locale packs, two pseudo-locales, 59 generated artifacts including paired Linux PO/MO resources, 26 passing tests, platform-format checks, and deterministic bundle ZIP SHA-256 `49ccf84b2afac15c604f74c45c6e7e72a1c12617eb5efdb3ada1d5164cd3c259`. Non-English packs remain explicitly unreviewed drafts. |
| Native Linux alpha.2 slice | Validated locally and remotely | Functional revision `60b560383e53bf4cf9ccc5ecf3821fe735206446` negotiates `bounded_text_document_v1`, converts bounded TXT/Markdown/CSV/JSON/HTML/SRT/WebVTT/DOCX/PPTX/XLSX/EPUB/PDF imports into Core jobs, preserves subtitle cue IDs/headers/timestamps and surfaces configurable cue-level line-length/reading-speed warnings plus localized PDF fidelity warnings without source content, CSV delimiters/quotes/variable-width rows/selected-column boundaries, JSON structure/path selection/escaping, HTML tags/attributes/links/scripts/styles/text nodes, OOXML/EPUB non-text resources and supported text nodes, PDF page association/coordinates and structured HTML fallback, sequentially translates pending prose segments with persisted progress, lists persisted jobs in the GTK queue, supports selection plus pause/resume/retry/cancel/export, and restores schema-14 snapshots without paths or credentials while retaining secure-provider, glossary, history, policy, and translation-memory behavior. Local 61/99-test suites, strict Clippy, Rust checks, and diff checks passed; Native `29656158543`, Foundation `29656158527`, and Flatpak `29656158526` passed. |
| Linux Flatpak packaging scaffold | Static validation passed; remote passed | Linux packaging revision `60b560383e53bf4cf9ccc5ecf3821fe735206446` publishes the pinned GNOME 49 manifest, immutable Core/Linux source pins including DOCX/PPTX/XLSX/EPUB/PDF warning UI, localized subtitle/PDF warning UI, and the queue UI, generated Cargo archive hashes for the current lockfile, desktop entry, AppStream metadata, icon, and constrained runtime permissions. `bash tools/validate-flatpak-metadata.sh` passed locally; `Flatpak Linux` run `29656158526` passed. Physical compositor/GPU rendering, signing, and distributable release remain unverified. |
| GitHub Actions | Passed | Core revision `81be0b8be9d7115b98eae3f134b4fd0f25411bbb` passed CI `29655212117` and Native SDK `29655212149`; Linux revision `60b560383e53bf4cf9ccc5ecf3821fe735206446` passed Native `29656158543`, Foundation `29656158527`, and Flatpak `29656158526`; l10n revision `738a7c7328f24acc12c15be78bb11737220bbbae` passed Localization `29656124100` and Foundation `29656124109`. |
| Non-functional repository heads | Published | Core head `81be0b8be9d7115b98eae3f134b4fd0f25411bbb`, l10n head `738a7c7328f24acc12c15be78bb11737220bbbae`, and Linux head `60b560383e53bf4cf9ccc5ecf3821fe735206446` are published functional pins and passed their remote gates. The release manifest remains unreleased with no artifacts. |
