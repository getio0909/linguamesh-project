...
| GitHub Actions | Passed | Linux functional run `29604269568` (job `87963611054`) and foundation run `29604269516` passed the exact fault test, persistent Secret Service lifecycle and secure-onboarding fixtures, no-credential loopback regression, GTK assertions, both display gates, bounded import path, and the all-feature build. Linux evidence head `b6d2503` passed Native Linux run `29627668119`, Foundation run `29627668093`, and Flatpak run `29627668108`; the native job also verified the pinned 117-message localization snapshot, Arabic RTL direction, source-buffer preservation, localized workspace widgets, active-provider/status-summary/partial-output/text-import/provider-profile/source-target/onboarding-stage/active-provider/notification/error/theme-option localization, prompted-flow store/delete rejection, real portal backend, application FileDialog callback, source-editor drag/drop gesture, dunst delivery, and visible viewable Dunst window. Central integration commit `522c94e` passed coordination run `29627851127`; the earlier failed status-localization attempts were corrected by updating the workflow localization pin. |
