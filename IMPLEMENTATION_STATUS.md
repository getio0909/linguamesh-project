...
incomplete because persistent desktop Secret Service lifecycle evidence and secure
...
| Native Linux alpha.2 slice | Validated locally and remotely | Functional revision `726508f8412727f8b14e32d27407487491f5e4cd` starts disconnected, derives provider setup from authoritative state, manages multiple profiles with independent models, preserves authenticated A/B routing without credential crossover, exposes baseline GTK accessibility semantics, uses a GIO Secret Service adapter with a corrected single-layer plain-string OpenSession Variant plus isolated real-daemon CRUD/cleanup verification and fail-closed session fallback, loads pinned English/Simplified Chinese PO catalogs for runtime Translate/Stop labels, emits generic completion notifications without source or translated content, and imports bounded UTF-8 TXT/Markdown text through GTK FileDialog/GIO or single-file source-editor drag-and-drop. Post-startup persistent Connect, model-update, and deletion `ENOSPC` failures reject before success, preserve the prior session, and restore only pre-fault state. Local suites passed 44 and 70 tests plus one exact fault test. Native Linux run `29600898951` (job `87952473459`) passed 75 library tests with 2 intentional ignores, the real GTK tests under X11/Xvfb and forced Wayland/headless Weston, the storage-fault gate, isolated Secret Service fixture, and native build. Persistent desktop keyring restoration, locked/prompted behavior, notification-server delivery, interactive portal leases, and complete UI gettext coverage remain unverified. |
| GitHub Actions | Passed | Linux functional run `29600898951` (job `87952473459`) and foundation run `29600898977` passed the exact fault test, isolated Secret Service CRUD fixture, GTK assertions, both display gates, bounded import path, and the all-feature build. Documentation evidence head `93f40456a53074ad437e4ee74634348c35afc049` passed Native Linux run `29601110914` (job `87953169459`) and foundation run `29601110906`. Central integration commit `acdf43cde6a7a971d1cb27e0c6a672b6b86cbc21` passed coordination run `29601473308`, Linux job `87954353885`, and PowerShell job `87954353964`. |
| Non-functional repository heads | Passed | Core head `b8ce1fda3a2a20ea2bc99bb6abff70e1a7a95383` is a documentation-only descendant of the functional pin; Core run `29573340721` and Native SDK run `29573340735` passed. Linux evidence head `93f40456a53074ad437e4ee74634348c35afc049` changes only evidence documentation relative to functional source `726508f8412727f8b14e32d27407487491f5e4cd`; foundation run `29601110906` and Native Linux run `29601110914` (job `87953169459`) passed. The release manifest intentionally pins functional revisions. |
...
checksum, owner, and remote invariants. Persistent desktop Secret Service restoration and locked/prompted
behavior, complete UI gettext coverage, secure persistent-credential onboarding, notification-server delivery, interactive portal leases, database faults beyond the verified `ENOSPC` transaction boundary, packaging,
...
Secret Service-backed desktop credential lifecycle, secure persistent-credential onboarding, compatible
