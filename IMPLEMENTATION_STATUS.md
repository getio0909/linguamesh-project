...
completions skip history writes. The latest Linux history-controls slice adds a bounded inspection
window, exact per-entry deletion, and deterministic escaped UTF-8 TSV export; translation memory and
history enable/disable policy remain later slices.
...
History inspection/export/per-entry deletion is no longer an open boundary for the Linux slice;
translation-memory stores, history enable/disable policy, and end-user prompt approval remain open.
...
| Rust core checkpoint | Validated locally and remotely | Functional revision `6079138348f3182b19c017f50db768df05da62cb` uses Core `0.1.0-alpha.2`, ABI 1, protocol 1, `protected_spans_v1`, `long_text_chunking_v1`, bounded request-level glossary protection, deterministic bounded glossary CSV interchange, and SQLite schema 3 bounded translation history with newest-first listing and exact per-entry deletion; it passed rustfmt, strict Clippy, full workspace tests, offline build, cargo-deny policy, native SDK checks, CI run `29638085207`, and Native SDK run `29638085241`. Long input is split on UTF-8-safe semantic boundaries, streamed sequentially, and cancelled through the shared token; the default 16 KiB limit is an approximate byte budget rather than a tokenizer budget. Canonical profiles, the bounded typed host-secret broker, Linux default-VFS SQLite no-follow protection, streamed protected-span restoration, glossary validation without credential-value persistence, and Incognito-aware history persistence are implemented. |
| Localization bundle | Validated locally and remotely | l10n evidence revision `971d1691a4eff396c71216b898e30fcfb23e72fa` contains 240 canonical messages, including Linux-only status, partial-output, text-import, glossary CSV import/export, Incognito and history controls, translation-export, provider-profile, source-target, onboarding-stage, active-provider, notification, draft-note, locale selector language names, fixed provider/file/worker errors, reducer-state/category guidance, fixed worker/file/storage/provider-error guidance, construction-stage provider/default-control and request-level glossary copy, and Core/loopback startup plus profile-storage error copy, 12 official locale packs, two pseudo-locales, 59 generated artifacts including paired Linux PO/MO resources, 26 passing tests, platform-format checks, and deterministic bundle ZIP SHA-256 `36f782f45a228f68520c691a7d015a4839531a892732e31119fd5c0e83a2be9c`. Non-English packs remain explicitly unreviewed drafts. |
| Native Linux alpha.2 slice | Validated locally and remotely | Functional revision `9ff8f3fb9cf61e1f51c3fc5e042e5fc8f601b837` negotiates Core `protected_spans_v1` and `long_text_chunking_v1`, carries bounded request-level glossary rules and explicit Incognito privacy policy into translation requests, persists bounded standard translation history with a clear-all control while skipping Incognito completions, refreshes the history count immediately after successful writes, and exposes newest-first inspection, exact per-entry deletion, and escaped UTF-8 TSV export, imports/exports fixed-schema glossary CSV through GTK FileDialog/GIO, and streams oversized requests in semantic-boundary chunks in order with cancellation propagation. It starts disconnected, derives provider setup from authoritative state, manages multiple profiles with independent models, preserves authenticated A/B routing without credential crossover, exposes baseline GTK accessibility semantics, uses a GIO Secret Service adapter with a corrected single-layer plain-string OpenSession Variant plus isolated real-daemon CRUD/cleanup, persistent daemon-restart restoration, locked-item fail-closed verification, secure persistent-credential onboarding, a no-credential OpenAI-compatible loopback path, and session fallback, exposes twelve official Linux PO/MO packs with runtime action-label, workspace-widget, active-provider, active-provider transition/mode, status summary/partial-output, text-file import/export, glossary CSV import/export, Incognito and history controls, provider-profile, source/target language, locale selector language names, onboarding-stage, draft-note, fixed provider/file/worker and reducer-state/category error messages, fixed worker/file/storage/provider-error guidance, construction-stage provider/default-control and request-level glossary copy, Core/loopback startup and profile-storage error copy, and theme-option switching plus Arabic RTL root direction, preserves the source editor buffer across a Simplified Chinese-to-Arabic locale switch, emits generic completion notifications without source or translated content, imports bounded UTF-8 TXT/Markdown through GTK FileDialog/GIO or single-file source-editor drag-and-drop, and asynchronously exports UTF-8 translated output with source-file overwrite protection. Post-startup persistent Connect, model-update, and deletion `ENOSPC` failures reject before success, preserve the prior session, and restore only pre-fault state. Local suites passed 54 portable tests and 83 demo-provider tests with one controlled ignore; source-level GUI checks passed locally while the binary GUI test is CI-only on this host. Native run `29638278667` (job `88064319932`), Foundation run `29638278676`, and Flatpak run `29638278677` (job `88064320034`) passed. |
...
| GitHub Actions | Passed | l10n revision `971d1691a4eff396c71216b898e30fcfb23e72fa` passed Localization run `29638057493` and Foundation run `29638057470`; Core functional revision `6079138348f3182b19c017f50db768df05da62cb` passed CI run `29638085207` and Native SDK run `29638085241`. Linux revision `9ff8f3fb9cf61e1f51c3fc5e042e5fc8f601b837` passed Native run `29638278667` (job `88064319932`), Foundation run `29638278676`, and Flatpak run `29638278677` (job `88064320034`). Central documentation revision `08250c870b42b6df249c30a0379a5db505ef23c2` passed coordination run `29638614216` (Linux job `88065175722`, PowerShell job `88065175707`). |
| Non-functional repository heads | Passed | Core head `6079138348f3182b19c017f50db768df05da62cb` is the functional glossary-CSV, long-text-chunking, Incognito-policy, and bounded-history pin and passed CI plus the full Native SDK matrix. Linux functional revision `9ff8f3fb9cf61e1f51c3fc5e042e5fc8f601b837` passed Native, Foundation, and Flatpak gates; documentation head `9ff8f3f` records the evidence. The release manifest intentionally pins functional revisions and lists no release artifact. |
