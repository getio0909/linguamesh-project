...
sandbox startup smoke; portal/notification delivery and release artifacts remain open. No stable product
...
| Linux Flatpak packaging scaffold | GNOME 49 SDK build and bounded sandbox startup validated | Linux packaging revision `fd1f400058f4c68b47a9bd0823e790c6d9cef263` publishes the pinned GNOME 49 manifest, immutable Core/Linux source pins, generated Cargo archive hashes, desktop entry, AppStream metadata, icon, and constrained runtime permissions. `bash tools/validate-flatpak-metadata.sh` passed locally; `Flatpak Linux` run `29608245156` (job `87976563401`) built `linguamesh-linux-x86_64-x86_64.flatpak`, uploaded artifact `8417803048` (2,395,628 bytes), installed it under Xvfb/private D-Bus, confirmed runtime `org.gnome.Platform/x86_64/49`, and passed the bounded startup smoke. Portal/notification delivery, signing, and distributable release remain unverified. |
| GitHub Actions | Passed | Linux functional run `29604269568` (job `87963611054`) and foundation run `29604269516` passed the exact fault test, persistent Secret Service lifecycle and secure-onboarding fixtures, no-credential loopback regression, GTK assertions, both display gates, bounded import path, and the all-feature build. Linux documentation evidence head `a255b039ce37bcb2b362cfeb3d34a6283ed2aad5` passed Native Linux run `29604459967` and foundation run `29604459891`. Packaging head `302ba8c` passed Native Linux run `29605298735` and foundation run `29605298772`; the GNOME 49 Flatpak packaging revision `fd1f400` passed `Flatpak Linux` run `29608245156` with job `87976563401`, including the sandbox startup smoke, while the same packaging change passed Native Linux run `29607653918` and Foundation run `29607653880`. Central integration commit `a7ca1f84eef3f98647a4383dc931c11049171b63` passed coordination run `29604649153` with Linux job `87964871597` and PowerShell job `87964871588`; the packaging documentation checkpoint passed coordination run `29605424763` with Linux job `87967368496` and PowerShell job `87967368478`; the current evidence sync commit `f2bc443` passed coordination run `29608669489` with Linux job `87977918468` and PowerShell job `87977918397`. |
...
database faults beyond the verified `ENOSPC` transaction boundary, portal/notification delivery,
